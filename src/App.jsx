...
function POS({T,t,css,prods,setProds,custs,emps,sales,setSales,udh,setUdh,dr,setDr,user,isAdmin,gid,pkr,td,log,buildBill,silentPrint,shopInfo,bk,setBk,webStock}) {
  ...
  const checkout=()=>{
    ...
    // Udhaar credit-limit guard
    if(cust&&cust!=="Walk-in"&&rem>0){const co=custs.find(c=>c.name===cust);const lim=+(co?.creditLimit||0);
      if(lim>0){const existing=udh.filter(u=>u.customerName===cust&&u.remaining>0).reduce((a,u)=>a+u.remaining,0);
        if(existing+rem>lim){
          if(!isAdmin){alert(`⚠️ ${cust} ki UDHAAR LIMIT (Rs.${lim.toLocaleString()}) cross ho rahi hai!\nMojooda udhaar: Rs.${existing.toLocaleString()} + naya: Rs.${rem.toLocaleString()}\nAdmin se permission lo ya zyada cash lo.`);return;}
          if(!confirm(`⚠️ ${cust} ki udhaar limit cross ho rahi hai (Rs.${(existing+rem).toLocaleString()} > Rs.${lim.toLocaleString()}). Phir bhi bill banayein?`))return;
        }}}
...
function Customers({T,t,css,custs,setCusts,sales,gid,pkr,log,td}) {
  ...
  const blank={name:"",phone:"",whatsapp:"",address:"",city:"Kunjah",notes:"",loyalty:"Silver",birthday:"",creditLimit:0,totalPurchases:0,udhaar:0,visits:0};const [fm,setFm]=useState(blank);
  const [showInactive,setShowInactive]=useState(false);
  const lastBuy=(name)=>{const ss=sales.filter(s=>s.customer===name);return ss.length?ss.map(s=>s.date).sort().slice(-1)[0]:null;};
  const daysSince=(d)=>d?Math.floor((Date.now()-new Date(d).getTime())/86400000):9999;
  const inactive=custs.filter(c=>{const lb=lastBuy(c.name);return c.name!=="Walk-in"&&lb&&daysSince(lb)>=60;}).map(c=>({...c,lb:lastBuy(c.name),days:daysSince(lastBuy(c.name))})).sort((a,b)=>b.days-a.days);
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px",flexWrap:"wrap",gap:"6px"}}><div style={css.h1}>👥 {t.customers}</div><div style={css.row}><button onClick={()=>setShowInactive(v=>!v)} style={css.btn(showInactive?"#a052e0":T.surface)}>😴 Win-back ({inactive.length})</button><button onClick={()=>setBc(true)} style={css.btn(T.info)}>📢 Broadcast</button><button onClick={()=>{setEc(null);setFm(blank);setSf(true);}} style={css.btn()}>+ Add</button></div></div>
      {showInactive&&<div style={{...css.card,borderLeft:`4px solid #a052e0`,marginBottom:"10px"}}><div style={{fontWeight:"700",color:"#a052e0",fontSize:"12px",marginBottom:"6px"}}>😴 60+ din se nahi aaye ({inactive.length}) — wapas bulao</div>{inactive.length===0?<div style={{fontSize:"11px",color:T.muted}}>Sab active hain! 🎉</div>:<div style={{maxHeight:"180px",overflow:"auto"}}>{inactive.map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",borderBottom:`1px solid ${T.border}33`,fontSize:"11px"}}><span>{c.name} <span style={{color:T.muted}}>· {c.days} din pehle · {pkr(c.totalPurchases)}</span></span><button onClick={()=>window.open(waLink(c,`Assalam o Alaikum ${c.name}! 🧵 Jameel Fabrics me nayi collection aa gayi hai. Aap bohot din se nahi aaye — is dafa aap ke liye special discount! Tashreef layen. 🙏`))} style={{...css.btn(T.success),fontSize:"10px",padding:"2px 8px"}}>📱 Wapas bulao</button></div>)}</div>}</div>}
